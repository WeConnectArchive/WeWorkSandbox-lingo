...
[![CircleCI](https://circleci.com/gh/weworksandbox/lingo.svg?style=svg&circle-token=b339a4092e79e4321bae61e5d9a40c38bfec96db)](https://circleci.com/gh/weworksandbox/lingo)
[![Coverage Status](https://coveralls.io/repos/github/weworksandbox/lingo/badge.svg?branch=master&t=AsP7i5)](https://coveralls.io/github/weworksandbox/lingo?branch=master)

Lingo is a library used for generating SQL queries for Go. It exists to help ease the pain of manually building SQL
queries using strings or other odd means like an ORM.

# Table of Contents
- Setup
        - [Config File](#config-file)
    - [Type Safe Columns](#type-safe-columns)
...
### Type Safe Columns
All the column types that are generated are configurable via the config file. The column types are only used on the
query building side. You do _not_ need to have an exact match of DB type to a Type Safe Column type.
...
### Database Support
Currently, only MySQL table structure parsing is supported. Note that you can build your own table files using
...
1. Use `Values()` passing in other columns or aggregate `Expression`s
1. Use `Values()` with the previous bullet point, but also constants using `expression.NewValue()` / `expression.NewValues()`
