...
const pkgCoreExpPath = "github.com/weworksandbox/lingo/pkg/core/expression/path"
const pkgSQLPath = "github.com/weworksandbox/lingo/pkg/core/sql"
...
// createToSQLFunc makes the ToSQL(d core.Dialect) function
//
// func (q qResourceGroup) ToSQL(d core.Dialect) (sql.Data, error) {
...
func createToSQLFunc(table string) *jen.Statement {
	...
	returnVals := []jen.Code{jen.Qual(pkgSQLPath, typeData), jen.Error()}

	f := jen.Func().Parens(receiver).Id(funcToSQL).Call(funcParam).Params(returnVals...)
	f.Block(jen.Return(jen.Qual(pkgCoreExpPath, funcExpandTable).Call(jen.Id(dialect), jen.Id(tableReceiver))))
	return f
}

// createGetAliasFunc makes the ToSQL(d core.Dialect) function
//
...
// createGetNameFunc makes the ToSQL(d core.Dialect) function
//
