...
const pkgLingoExpPath = pkgLingo + "/expr/path"
const pkgSQLPath = pkgLingo + "/sql"
...
func createToSQLFunc(table string) *jen.Statement {
	...
	f.Block(jen.Return(jen.Qual(pkgLingoExpPath, funcExpandTable).Call(jen.Id(dialect), jen.Id(tableReceiver))))
	return f
}
